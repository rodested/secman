...
The module uses the passlib library for encryption and decryption.
...
HEADER_DISCLAIMER = "# Generated by secman.py. Do not edit manually, unless you know what you are doing"
...
def encrypt_secrets(file_path, master_key_env, master_key=None):
    ...
    print("Note that variables with empty values are left in the file but will not be encrypted.")
    if not master_key:
    ...
    with open(file_path, "w") as file:
        ...
        for line in lines:
            ...
            if secret_name == "MASTER_KEY_ENV":
                ...
                if secret_value:
                    print(f"Skipping {secret_name}: already encrypted in the file.\n        To re-encrypt it, delete the line and run the script again")
            else:
                ...
                count_encrypted += 1
                print(f"Encrypted {secret_name}. Original unencrypted value has been removed from the file.")
...
def set_master_key(file_path, master_key):
    """
    Set the MASTER key value in the target file
    """
    with open(file_path, "r") as file:
        lines = file.readlines()
    with open(file_path, "w") as file:
...
def decrypt_value(value, master_key):
    ...
    """
    fernet = Fernet(base64.urlsafe_b64encode(pbkdf2_sha256.hash(master_key.encode()).encode()))
